List<Map<String, dynamic>> videoPosts = [
  {
    'name': 'Sexto 6° Simposio',
    'videoUrl': 'assets/videos/0.mp4',
    'likes': 13992,
    'views': 1235841,
  },
  {
    'name': 'Zayn Malik Primer concierto como solista',
    'videoUrl': 'assets/videos/1.mp4',
    'likes': 23230,
    'views': 1523,
  },
  {
    'name': 'TE AMO SIN FE, SIN MEDIDA',
    'videoUrl': 'assets/videos/2.mp4',
    'likes': 24230,
    'views': 1343,
  },
  {
    'name': 'Gatitos',
    'videoUrl': 'assets/videos/3.mp4',
    'likes': 21564320,
    'views': 123563,
  },
  {
    'name': 'LIAM PAYNE 💔',
    'videoUrl': 'assets/videos/4.mp4',
    'likes': 320,
    'views': 2300,
  },
  {
    'name': 'TU BODA',
    'videoUrl': 'assets/videos/5.mp4',
    'likes': 3230,
    'views': 31030,
  },
  {
    'name': 'Ingeniera :)',
    'videoUrl': 'assets/videos/6.mp4',
    'likes': 10,
    'views': 330,
  },
  {
    'name': 'Escaleras Electricas',
    'videoUrl': 'assets/videos/7.mp4',
    'likes': 1320,
    'views': 33032,
  },
  {
    'name': 'Flores BONITAS!!',
    'videoUrl': 'assets/videos/8.mp4',
    'likes': 13993,
    'views': 3332,
  },
  {
    'name': 'VIDEOS QUE RELAJAAAAAAN!',
    'videoUrl': 'assets/videos/10.mp4',
    'likes': 2002,
    'views': 3332,
  },
  {
    'name': 'COVID-19! Pandemia Mundial😫',
    'videoUrl': 'assets/videos/11.mp4',
    'likes': 1998,
    'views': 3332,
  },
  {
    'name': 'Sombras!',
    'videoUrl': 'assets/videos/12.mp4',
    'likes': 1977,
    'views': 3332,
  },
  {
    'name': 'Mujer acomodando su librero :)!',
    'videoUrl': 'assets/videos/13.mp4',
    'likes': 342,
    'views': 3332,
  },
  {
    'name': 'Viajando por carretera',
    'videoUrl': 'assets/videos/14.mp4',
    'likes': 1997,
    'views': 3332,
  },
  {
    'name': 'Conectando con la naturaleza',
    'videoUrl': 'assets/videos/15.mp4',
    'likes': 1000,
    'views': 3332,
  },
  


];


